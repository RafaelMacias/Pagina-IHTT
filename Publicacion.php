...
class Publicacion{
	...
	function printHTML() {
		...
		$file = $header.file_get_contents("template/Publicacion.html").$footer;
		$file = str_replace("@@CURRENTSITE@@", "Publicación", $file);
		$file = str_replace("@@PAGESCRIPT@@", '<script type="text/javascript" src="@@DIR@@js/Publicacion.js"></script>', $file);
		...
		$file = str_replace("@@EMAIL@@", $Epublicacion, $file);
		...
		if ($admin==0) {
			...
			if ($publicacion['Nombre_Imagen']==null) {
				...
				$figure = '<figure '.$class_posicion.'><img src="Documentos/Imagenes/Noticias/'.$publicacion['Nombre_Imagen'].'" alt="daisy photographed from below"></figure><br>';
			}
			...
			if ($publicacion['Nombre_Imagen']==null) {
				...
				$figure = '<figure '.$class_posicion.'><img src="Documentos/Imagenes/Noticias/'.$publicacion['Nombre_Imagen'].'" alt="daisy photographed from below"><figcaption><a><i class="fas fa-edit mr-2 edit-icon" onclick="modificarImagenCreada();"></i></a></figcaption></figure><br>';
			}
