...
function select_pw($q,$p) {
	...
	function printHTML() {
		$header = file_get_contents("template/hearder2.html");
		require_once("config.php");
		$footer = file_get_contents("template/footer.html");
		
		$file = $header.file_get_contents("template/Prueba.html").$footer;
    	...
    	for($i=0;$i<count($data);$i++){
			...
			if ($i==0) {
		 	...
		 	$result .= '<div class="carousel-item '.$class.'">
                            <img class="d-block w-100" src="'.$dirimagen.'" alt="Fifth slide">
                        </div>'
		;}
